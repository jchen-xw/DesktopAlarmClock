...
void OLED_W_SCL(uint8_t BitValue)
{
    ...
    HAL_GPIO_WritePin( SCL_PB8_GPIO_Port, SCL_PB8_Pin, ( GPIO_PinState )BitValue );
...
void OLED_W_SDA(uint8_t BitValue)
{
    ...
    HAL_GPIO_WritePin( SDA_PB9_GPIO_Port, SDA_PB9_Pin, ( GPIO_PinState )BitValue );
...
/**
  ...
  * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
void OLED_ShowConstString(uint8_t X, uint8_t Y, const char *str, uint8_t FontSize)
{
	uint8_t i;
...
/*************************Flappy Bird function start****************************************/
void OLED_DrawRectangle_int16(int16_t X, int16_t Y, uint8_t Width, uint8_t Height, uint8_t IsFilled)
{
	int16_t i, j;
	if (!IsFilled)		//指定矩形不填充
	{
...
void OLED_Test(void)
{
	...
	/*在(8, 7)位置显示字符'B'，字体大小为6*8点阵*/
	OLED_ShowChar( 8, 7, 'B', OLED_6X8 );
	
	/*在(0, 16)位置显示字符串"Hello World!"，字体大小为6*8点阵*/
